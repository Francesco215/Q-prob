...
class Agent:
    def __init__(self, obs_shape: tuple, action_dim: int, max_action: float, pixel_obs: bool, discrete: bool,
        ...
        self.value = Gumbel(self.zsa_dim, self.zs_dim, self.value_hdim, self.value_activ).to(self.device)
    ...
    def train_rl(self, state: torch.Tensor, action: torch.Tensor, next_state: torch.Tensor,
        ...
        # Freeze encoder and value networks
        ...
        for p in self.value.parameters(): p.requires_grad = True
